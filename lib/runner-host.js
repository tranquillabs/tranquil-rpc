...
//   - the grants a run was approved for travel with its token, and gate which capabilities the
//     session exposes; the child cannot ask for more than the header declared
...
function handleConnection(socket) {
  ...
  const onAuthMessage = (data) => {
    ...
    const session = new RpcSession(
      ...
      buildHostApi({ kind: "runner", runId, scriptPath, scriptDir, grants, subscriptions })
    );
    ...
    // Establishing and closing a session is the expected path of every automation run, so
    // neither is logged: a pair of lines per run, keyed by a runId nothing else prints, was the
    // last routine chatter left in the dev terminal. What remains logged here is the server
    // coming up (once per window) and the two ways this can actually go wrong — a refused auth
    // and a server error.
    socket.send("AUTH OK");
  };
...
export function mintRunToken({ runId, scriptPath, scriptDir, grants = null, debug = false }) {
