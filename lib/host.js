...
import { closeRunnerServer } from "./runner-host.js";
...
function manage(webview, item) {
  let current = null; // { transport, session, subscriptions }
  ...
  const teardown = (reason) => {
    ...
    try {
      // Dispose anything a capability registered for this session (e.g. pane-control
      // buttons holding page-side action stubs), so reloads don't accumulate them.
  ...
  const onLoad = () => {
    ...
    if (!isTrusted(url)) {
      // Default-deny: untrusted pages receive no runtime and no session at all. Not logged —
      // this is the expected path for every ordinary page load, so logging it here means a
      // line per navigation per tab. The interesting events are the rare ones below: a
      // trusted session actually opening, or an injection failing.
      return;
    }

    const transport = hostTransport(webview);
    // Session-scoped disposables: capabilities register page-lifetime resources here (see
    // buildHostApi ctx), and teardown disposes them on reload/close.
    const subscriptions = new CompositeDisposable();
    const session = new RpcSession(
      transport,
      buildHostApi({ kind: "webview", item, webview, url, subscriptions })
    );
    current = { transport, session, subscriptions };
    webview
...
export function activate() {
  ...
  // Roots are registered by other packages after we activate, so reporting them here would
  // always print an empty list. addTrustedRoot() logs each one as it actually arrives.
